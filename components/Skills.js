function Skills() {
    try {
        ...
        return (
            <section id="skills" data-name="skills" data-file="components/Skills.js" className="py-20 bg-white">
                <div className="container mx-auto px-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
                        {skills.map((skill, index) => (
                            <div 
                                key={index} 
                                className={`fade-in text-center cursor-pointer transition-all duration-300 p-6 rounded-xl ${
                                    hoveredSkill === index 
                            ...
                            >
                                <div className={`w-16 h-16 mx-auto mb-4 bg-${skill.color}-100 rounded-full flex items-center justify-center`}>
                                    <i className={`${skill.icon} text-4xl text-${skill.color}-600 ${
                                        hoveredSkill === index ? 'animate-bounce' : ''
                                    }`}></i>
                                </div>
                                
                                <h3 className={`text-lg font-semibold transition-colors duration-300 ${
                                    hoveredSkill === index ? 'text-purple-600' : 'text-gray-800'
                                }`}>
                                    {skill.name}
                                </h3>
                            </div>
